    ...
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-grey-400'>
        ...
        {token ? <div className='flex items-center gap-2 cursor-pointer group relative'> 
            ...
            <div className="absolute top-0 right-0 pt-14 text-base font-medium  z-20 hidden group-hover:block">
                <div className="min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4 text-grey-600">
                    <p onClick={()=>navigate('my-profile')} className=' hover:text-back cursor-pointer'>My Profile</p>
                    <p  onClick={()=>navigate('my-appointments')}  className=' hover:text-back cursor-pointer'>My Appointments</p>
                    <p onClick={logout} className=' hover:text-back cursor-pointer'>Log out</p>
                </div>
            </div>
        </div>:
